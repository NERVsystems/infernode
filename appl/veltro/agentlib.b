...
tooldesc(name: string): string
{
	...
	"present"  => return "Manage the Lucifer presentation zone: create <id> [type=markdown|text|table|code|pdf|image|mermaid] [label=text], write <id> <content-or-path>, center <id>, list, status. For mermaid: write raw Mermaid syntax (NOT fenced code blocks). For pdf/image: write the file path.";
