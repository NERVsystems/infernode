...
init(nil: ref Draw->Context, args: list of string)
{
	...
	winm = load Xenithwin Xenithwin->PATH;
	winm->init();
...
execdiff(w: ref Wiki, s: string)
{
	...
	dup(fd.fd, 0);
	fd = open(sprint("/mnt/xenith/%d/body", w.win.winid), OWRITE);
	dup(fd.fd, 1);
	dup(fd.fd, 2);
	fd = nil;
	pctl(NEWFD, 0 :: 1 :: 2 :: nil);
	sh->system(nil, "/xenith/wiki/wiki.diff '" + s + "'");
}
...
xenithevent(wiki: ref Wiki, e: ref Event)
{
...
expand(w: ref Win, e: ref Event, exenith: ref Event): ref Event
{
	(n, q0, q1) := getdot(w);
...
{
	e := ref Event;
	e.b = array[Xenithwin->EVENTSIZE*UTFmax+1] of byte;
	e.r = array[Xenithwin->EVENTSIZE+1] of int;
	return e;
}
