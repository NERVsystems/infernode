...
main(argv : list of string)
{
	...
	id = int string buf;
	buf = nil;
	env->setenv("xenithwin", string id);
	...
	c <-= 1;
	fd = open("/dev/xenith/data", ORDWR);
	if(fd == nil)
		error("/dev/xenith/data");
	wd  := workdir->init();
	# b = sprint("name %s/-%s\n0\n", wd, name);
	b = sprint("name %s/-%s\n", wd, name);
	ab := array of byte b;
...
run(argv : list of string, id : int, c : chan of int)
{
	...
	<-c;
	pctl(FORKNS, nil);
	if(bind("/dev/xenith/data1", "/dev/cons", MREPL) < 0){
	...
	fd0 = fd1 = nil;
	b := sprint("/chan/%d", id);
	if(bind(b, "/dev/xenith", MREPL) < 0)
		error("bind /dev/xenith");
