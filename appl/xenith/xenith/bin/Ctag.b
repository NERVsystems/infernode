...
{
	io := bufio->open("/mnt/xenith/index", Sys->OREAD);
	if(io == nil){
		fprint(sys->fildes(2), "couldn't open xenith/index\n");
		return 0;
	}
	while((s := io.gets('\n')) != nil){
		fwin := int s[0:11];
...
highlight(f: string, addr: string): int
{
	...
	afd := open(sprint("/mnt/xenith/%d/addr", fwin), Sys->OWRITE);
	cfd := open(sprint("/mnt/xenith/%d/ctl", fwin), Sys->OWRITE);
	if(afd == nil || cfd == nil)
		return 0;
	fprint(afd, "%s", addr);
	fprint(cfd, "dot=addr");
