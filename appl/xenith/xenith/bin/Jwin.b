...
init(nil:ref Draw->Context, args:list of string)
{
	...
	xenithobj := mkhostobj(exec, "Xenithwin");
	es->put(exec, exec.this, "Xenithwin", es->objval(xenithobj));

	{
		xenithwin = w := Win.wnew();
		w.wname("Jwin");
...
doexec(nil: ref Win, cmd: string, xenithobj: ref Obj): int
{
	cmd = skip(cmd, "");
	arg: string;
	...
	"Del" or "Delete" =>
		return -1;
	* =>
		oscript := getobj(exec, xenithobj, "onexec");
		va := array[2] of ref Val;
		va[0] = es->strval(cmd);
		va[1] = es->strval(arg);
		if(oscript != nil){
			v := es->call(exec, oscript, xenithobj, va, 1).val;
...
dolook(nil: ref Win, cmd: string, xenithobj: ref Obj): int
{
	oscript := getobj(exec, xenithobj, "onlook");
...
mainwin(w: ref Win, xenithobj: ref Obj)
{
	c := chan of Event;
	na: int;
	ea: Event;
...
#		ans = es->valref(es->strval(s));
	...
	"System.prototype.postUrl" =>
		...
		ans = es->valref(es->strval(s));
	"Xenithwin.prototype.writebody" =>
		s : string;
		for (ai := 0; ai < len args; ai++)
				s += es->toString(ex, es->biarg(args, ai));
		xenithwin.wwritebody(s);
	"Xenithwin.prototype.read" =>
		m := es->toInt32(ex, es->biarg(args, 0));
		n := es->toInt32(ex, es->biarg(args, 1));
		s := xenithwin.wread(m, n);
		ans = es->valref(es->strval(s));
	"Xenithwin.prototype.name" =>
		s := es->toString(ex, es->biarg(args, 0));
		xenithwin.wname(s);
	"Xenithwin.prototype.tagwrite" =>
		s := es->toString(ex, es->biarg(args, 0));
		xenithwin.wtagwrite(s);
	"Xenithwin.prototype.clean" =>
		xenithwin.wclean();
	"Xenithwin.prototype.setaddr" =>
		s :=  es->toString(ex, es->biarg(args, 0));
		n := xenithwin.wsetaddr(s, 1);
		ans = es->valref(es->numval(real n));
	"Xenithwin.prototype.replace" =>
		s := es->toString(ex, es->biarg(args, 0));
		t := es->toString(ex, es->biarg(args, 1));
		xenithwin.wreplace(s, t);
	"Xenithwin.prototype.select" =>
		s := es->toString(ex, es->biarg(args, 0));
		xenithwin.wselect(s);
	"Xenithwin.prototype.readall" =>
		ans = es->valref(es->strval(xenithwin.wreadall()));
	* =>
