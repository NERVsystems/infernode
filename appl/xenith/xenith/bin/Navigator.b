...
getwidth(w: ref Win)
{
	if(ctxt == nil || draw == nil)
		return;
	if((fd := open(sprint("/mnt/xenith/%d/ctl", w.winid), Sys->ORDWR)) == nil){
		sys->fprint(sys->fildes(2), "Navigator: couldn't open /dev/xenith/ctl %r\n");
		return;
	}
	buf := array[256] of byte;
	if((n := read(fd, buf, len buf)) <= 0){
		sys->fprint(sys->fildes(2), "Navigator: couldn't read /dev/xenith/ctl %r\n");
		return;
	}
