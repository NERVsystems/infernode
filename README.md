...
## Windows Installation (Pre-built)

Download the latest `InferNode-windows-amd64.zip` from the [releases page](https://github.com/NERVsystems/infernode/releases), extract it, and open the `InferNode` folder.

1. **Run setup** - Double-click `setup-windows.bat`. Choose between a local Ollama model (free, private) or an Anthropic API key. Setup installs Ollama automatically if needed.
2. **Launch InferNode** - Double-click `InferNode.bat`. The Lucifer GUI opens.

That's it. Ollama runs in the background; the LLM service (`llmsrv`) starts automatically inside InferNode.

See [docs/WINDOWS-BUILD.md](docs/WINDOWS-BUILD.md) for building from source.
...
| AMD64 Windows | Intel/AMD x86_64 | **14x** | SDL3 GUI with D3D acceleration |
...
- **Windows AMD64 Port** - Headless and SDL3 GUI with JIT compiler (14x speedup), pre-built distribution available
