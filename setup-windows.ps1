...
function Setup-Anthropic {
    ...
    # Write llm.cfg so the profile knows the backend
    $cfgDir = Join-Path $Root "lib\veltro"
    if (-not (Test-Path $cfgDir)) { New-Item -ItemType Directory -Path $cfgDir -Force | Out-Null }
    $cfgPath = Join-Path $cfgDir "llm.cfg"
    Set-Content -Path $cfgPath -Value "# Veltro LLM configuration - generated by setup-windows.ps1`nbackend=api" -NoNewline
    ...
    Write-Host "  Model: claude-sonnet-4-5 (configurable at runtime)" -ForegroundColor White
}
...
function Setup-Ollama {
    ...
    if (-not (Test-Path $cfgDir)) { New-Item -ItemType Directory -Path $cfgDir -Force | Out-Null }
    $cfgPath = Join-Path $cfgDir "llm.cfg"
    $cfgText = "# Veltro LLM configuration - generated by setup-windows.ps1`nbackend=openai`nurl=http://127.0.0.1:11434/v1`nmodel=$model"
...
# Launch instructions
$emuPath = Join-Path $Root "o.emu.exe"
if (Test-Path $emuPath) {
    Write-Host "To launch InferNode:" -ForegroundColor White
    Write-Host "  cd $(Split-Path $emuPath -Parent)" -ForegroundColor DarkGray
    Write-Host "  .\o.emu.exe -r . wm/wm          # graphical (requires SDL3.dll)" -ForegroundColor DarkGray
    Write-Host "  .\o.emu.exe -r . sh -l           # headless shell" -ForegroundColor DarkGray
} else {
    Write-Host "To launch InferNode, run o.emu.exe from your InferNode directory:" -ForegroundColor White
    Write-Host "  .\o.emu.exe -r . wm/wm           # graphical (requires SDL3.dll)" -ForegroundColor DarkGray
    Write-Host "  .\o.emu.exe -r . sh -l            # headless shell" -ForegroundColor DarkGray
}
Write-Host ""
